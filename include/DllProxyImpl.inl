...
#error This file requires a compiler with C++20 support or newer for constexpr, constinit, and concept features.
#endif
...
#define DLL_PROXY_ALIAS_SYMBOL(From, To)                                       \
  __pragma(comment(linker, "/ALTERNATENAME:" To "=" From))
#define DLL_PROXY_EXPORT_SYMBOL(From, To)                                      \
  __pragma(comment(linker, "/EXPORT:" To "=" From))
#define DLL_PROXY_EXPORT_ORDINAL(From, To, Ordinal)                            \
  __pragma(comment(linker, "/EXPORT:" To "=" From ",@" #Ordinal))
#elif defined(__GNUC__) // _MSC_VER
#define DLL_PROXY_MAKE_SECTION(Name)                                           \
  __asm__(".pushsection \"" Name "\", \"xr\"\n.balign 4096\n.popsection\n");
...
#endif
#define DLL_PROXY_EXPORT_SYMBOL(From, To)                                      \
  DLL_PROXY_ALIAS_SYMBOL(From, DLL_PROXY_SYMBOL_PREFIX To)
#define DLL_PROXY_EXPORT_ORDINAL(From, To, Ordinal)                            \
  DLL_PROXY_ALIAS_SYMBOL(From, DLL_PROXY_SYMBOL_PREFIX To)
#else // __GNUC__
...
alignas(DefaultPageAlign) DLL_PROXY_USE_SECTION(".dllprox$a") constinit uint8_t
    ExportBoundaryStart = 1;
...
static_assert(sizeof(X86StubPlaceholderCode) == 16,
              "Opcodes are expected to be 16 bytes");
#pragma pack(pop)

#define MAKE_PROXY_EXPORT_IMPL(PlaceholderName)                                \
  extern "C" alignas(DefaultFuncAlign) DLL_PROXY_USE_SECTION(                  \
      ".dllprox$b") constinit X86StubPlaceholderCode PlaceholderName{          \
      0xFF,                                                                    \
      0x25,                                                                    \
      &PlaceholderName.JmpDwordDestination,                                    \
      (void *)&Internal::UnresolvedExportCallback,                             \
      0xCC,                                                                    \
      0xCC,                                                                    \
      0xCC,                                                                    \
      0xCC,                                                                    \
      0xCC,                                                                    \
      0xCC};                                                                   \
  DLL_PROXY_ALIAS_SYMBOL(DLL_PROXY_STRINGIFY(_##PlaceholderName),              \
                         #PlaceholderName)
...
static_assert(sizeof(Amd64StubPlaceholderCode) == 16,
              "Opcodes are expected to be 16 bytes");
#pragma pack(pop)

#define MAKE_PROXY_EXPORT_IMPL(PlaceholderName)                                \
  extern "C" alignas(DefaultFuncAlign) DLL_PROXY_USE_SECTION(                  \
      ".dllprox$b") constinit Amd64StubPlaceholderCode PlaceholderName{        \
      0xFF, 0x25, 0, (void *)&Internal::UnresolvedExportCallback, 0xCC, 0xCC};
...
#define MAKE_PROXY_API_ORDINAL_IMPL(FuncName, Ordinal, VariableAlias)          \
  MAKE_PROXY_EXPORT_IMPL(VariableAlias)                                        \
  DLL_PROXY_EXPORT_ORDINAL(DLL_PROXY_STRINGIFY(VariableAlias), FuncName,       \
                           Ordinal)
...
#define DECLARE_PROXIED_API_ORDINAL(FuncName, Ordinal)                         \
  MAKE_PROXY_API_ORDINAL_IMPL(FuncName, Ordinal,                               \
                              DLL_PROXY_CONCAT(XPROXY_, __COUNTER__))
...
alignas(DefaultPageAlign)
    DLL_PROXY_USE_SECTION(".dllprox$z") constinit uint8_t ExportBoundaryEnd = 1;
...
extern "C" constexpr PIMAGE_TLS_CALLBACK DllProxyTLSCallback =
    Internal::TLSInitCallback;
...
//
namespace DllProxy::Internal {
constexpr PfnRealLibraryResolver RealLibraryResolver =
    ::DLL_PROXY_LIBRARY_RESOLVER_CALLBACK;
constexpr PfnRealExportResolver RealExportResolver =
    ::DLL_PROXY_EXPORT_RESOLVER_CALLBACK;
constexpr PfnExceptionCallback ExceptionCallback =
    ::DLL_PROXY_EXCEPTION_CALLBACK;
...
__declspec(noinline) void *GetLocalModuleHandle() {
  ...
  if (!moduleHandle) {
    uint32_t flags = GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                     GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT;
    HMODULE handle = nullptr;

    if (!GetModuleHandleExW(
            flags, reinterpret_cast<LPCWSTR>(&GetLocalModuleHandle), &handle))
...
template <typename T> T ConvertModuleRva(uintptr_t Rva) {
  return (T)(reinterpret_cast<uintptr_t>(GetLocalModuleHandle()) + Rva);
}

// Pass the callback as a templated type to avoid the need for std::function's
// allocations
template <typename T>
  requires(
      std::is_invocable_v<T, uint32_t /* Ordinal */, const char * /* Name */,
                          uintptr_t /* Address */>)
void EnumerateLocalModuleExports(T &&Callback) {
  ...
  auto ntHeaders =
      ConvertModuleRva<const IMAGE_NT_HEADERS *>(dosHeader->e_lfanew);
  ...
  auto exportDataDirectory =
      &ntHeaders->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT];
  auto exportDirectory = ConvertModuleRva<const IMAGE_EXPORT_DIRECTORY *>(
  ...
  auto exportAddressTable =
      ConvertModuleRva<const uint32_t *>(exportDirectory->AddressOfFunctions);
  auto exportNameTable =
      ConvertModuleRva<const uint32_t *>(exportDirectory->AddressOfNames);
  auto exportNameOrdinalTable = ConvertModuleRva<const uint16_t *>(
  ...
  for (uint32_t i = 0; i < exportDirectory->NumberOfFunctions; i++) {
    ...
    Callback(exportOrdinal, exportName,
             ConvertModuleRva<uintptr_t>(exportAddressTable[i]));
  }
}
...
void InitializeImpl() {
  ...
  const auto sectionStart =
      reinterpret_cast<uintptr_t>(&Section::ExportBoundaryStart);
  const auto sectionEnd =
      reinterpret_cast<uintptr_t>(&Section::ExportBoundaryEnd);
  const auto sectionLength = sectionEnd - sectionStart;
  DWORD oldProtection = 0;

  if (!VirtualProtect(reinterpret_cast<void *>(sectionStart), sectionLength,
                      PAGE_READWRITE, &oldProtection))
    UnrecoverableError(ErrorCode::VirtualProtectFailed);

  // Iterate over all exports from this DLL. If the RVA points into the dllprox
  // section, patch the jump to point back to the original library.
  EnumerateLocalModuleExports([&](uint32_t Ordinal, const char *Name,
                                  uintptr_t Address) {
    if (Address > sectionStart && Address < sectionEnd) {
      void *functionPointer = nullptr;
      const bool status =
          RealExportResolver(originalModule, Ordinal, Name, &functionPointer);
      ...
      // functionPointer is allowed to be null as long as success is reported.
      // Users can hurt themselves as they please.
      if (status) {
#if defined(_M_IX86)
        auto ptr = &reinterpret_cast<Section::X86StubPlaceholderCode *>(Address)
                        ->JmpDwordDestination;

        static_assert(sizeof(*ptr) == sizeof(LONG),
                      "Expected pointer to be 32 bits");
        InterlockedExchange(reinterpret_cast<LONG *>(ptr),
                            reinterpret_cast<LONG>(functionPointer));
#elif defined(_M_X64) // _M_IX86
        auto __unaligned ptr =
            &reinterpret_cast<Section::Amd64StubPlaceholderCode *>(Address)
                 ->JmpQwordDestination;

        static_assert(sizeof(*ptr) == sizeof(LONGLONG),
                      "Expected pointer to be 64 bits");
        InterlockedExchange64(reinterpret_cast<LONGLONG *>(ptr),
                              reinterpret_cast<LONGLONG>(functionPointer));
#endif                // _M_X64
      }
    }
  });
  ...
  FlushInstructionCache(GetCurrentProcess(),
                        reinterpret_cast<void *>(sectionStart), sectionLength);
}
} // namespace DllProxy::Internal
...
namespace DllProxy {
#if !defined(DLL_PROXY_TLS_CALLBACK_AUTOINIT)
void Initialize() { Internal::InitializeImpl(); }
#endif

void *DefaultLibraryResolverCallback() {
  // Try to load the DLL with the user-specified path. In the event it's not
  // found, there's nothing left to do here.
  HMODULE libraryHandle = LoadLibraryW(Section::OriginalLibraryName);

  if (!libraryHandle)
    return nullptr;

  // Prevent infinite recursion when module names collide. Make sure this DLL
  // doesn't grab a reference to itself.
  if (libraryHandle == Internal::GetLocalModuleHandle()) {
    FreeLibrary(libraryHandle);

    // Try system32 instead
    const size_t bufferLength = 4096;
    auto buffer = reinterpret_cast<wchar_t *>(
        VirtualAlloc(nullptr, bufferLength * sizeof(wchar_t),
...
bool DefaultExportResolverCallback(void *Module, uint32_t Ordinal,
                                   const char *Name, void **FunctionPointer) {
  ...
  if (!pointer)
    pointer = reinterpret_cast<void *>(GetProcAddress(
        static_cast<HMODULE>(Module), MAKEINTRESOURCEA(Ordinal)));
...
void DefaultExceptionCallback(ErrorCode Code) {
  ...
  TerminateProcess(GetCurrentProcess(),
                   0xBADC0DE0 | static_cast<uint8_t>(Code));
}
} // namespace DllProxy
